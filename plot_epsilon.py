# -*- coding: utf-8 -*-
"""
Created on Mon Jan 24 13:49:27 2022
...
for iseed in range(len(seed1)):
    ...
    for iuc,uc in enumerate(ucs):
        ...
        plt.figure(21)
        plt.loglog(ts*0.005,R,'o-',color=colors[iuc],label='uc='+str(uc))
        plt.legend()
        plt.xlabel('t')
        plt.ylabel('R(t)')
        plt.xlim([1e2,1e5])
        ...
        plt.figure(22)
        plt.loglog(ts[1:]*0.005,np.diff(R)/np.diff(ts*0.005)/R[0:-1],'o-',color=colors[iuc],label='uc='+str(uc))
        plt.legend()
        plt.xlabel('t')
        plt.ylabel(r'$\dot{\epsilon}(t)$')
        plt.xlim([1e2,1e5])
        # plt.ylim([10,1e5])
plt.figure(20)
...
plt.figure(22)
plt.plot(ts[90:]*0.005,ts[90:]**(-1)*300,'k--')
plt.plot([4e3,4e3],[1e-5,1e-2],'k:')
        # plt.figure(30)
